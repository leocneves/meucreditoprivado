...
const AssetPage: React.FC = () => {
  ...
  if (!asset) return <div className="p-10 text-center">Ativo não encontrado. <Link to="/" className="text-blue-500 underline">Voltar</Link></div>;
  ...
  return (
    <div className="container mx-auto px-4 py-8 space-y-8">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start gap-4">
        <div>
          <h1 className="text-4xl font-black text-slate-900">{asset.ticker}</h1>
          <p className="text-xl text-slate-500 font-medium">{asset.issuer_name}</p>
        </div>
        <button 
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-8">
          <ChartComponent prices={prices} ticker={asset.ticker} />
          
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex items-center gap-4">
              <div className="p-3 bg-blue-50 text-blue-600 rounded-lg"><Percent size={24} /></div>
              <div>
                <p className="text-xs text-slate-400 font-bold uppercase tracking-wider">Indexador</p>
                <p className="text-lg font-bold text-slate-800">{asset.coupon_type}</p>
              </div>
            </div>
            <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex items-center gap-4">
              <div className="p-3 bg-indigo-50 text-indigo-600 rounded-lg"><Calendar size={24} /></div>
              <div>
                <p className="text-xs text-slate-400 font-bold uppercase tracking-wider">Vencimento</p>
                <p className="text-lg font-bold text-slate-800">{asset.maturity_date}</p>
              </div>
            </div>
            <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex items-center gap-4">
              <div className="p-3 bg-green-50 text-green-600 rounded-lg"><ShieldCheck size={24} /></div>
              <div>
                <p className="text-xs text-slate-400 font-bold uppercase tracking-wider">Garantia</p>
                <p className="text-lg font-bold text-slate-800">{asset.guarantee}</p>
              </div>
            </div>
            <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex items-center gap-4">
              <div className="p-3 bg-purple-50 text-purple-600 rounded-lg"><FileText size={24} /></div>
              <div>
                <p className="text-xs text-slate-400 font-bold uppercase tracking-wider">Sênioridade</p>
                <p className="text-lg font-bold text-slate-800">{asset.seniority}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-slate-900 text-white p-8 rounded-3xl shadow-xl space-y-6">
          <h3 className="text-xl font-bold border-b border-slate-700 pb-4">Detalhes do Emissor</h3>
